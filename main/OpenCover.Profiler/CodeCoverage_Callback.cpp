...
UINT_PTR CCodeCoverage::FunctionMapper2(FunctionID functionId, void* clientData, BOOL* pbHookFunction)
{
    ...
    *pbHookFunction = FALSE;
    ...
    {
        ULONG uniqueId;
        if (profiler->_host->TrackMethod(functionToken, modulePath.c_str(), 
            profiler->m_allowModulesAssemblyMap[modulePath].c_str(), uniqueId))
        {
            *pbHookFunction = TRUE;
            retVal = uniqueId;
        }
...
{
    _host->AddTestEnterPoint(gsl::narrow<ULONG>(clientData));
}
...
{
    _host->AddTestLeavePoint(gsl::narrow<ULONG>(clientData));
}
...
void CCodeCoverage::FunctionTailcall2(
    ...
    /*[in]*/COR_PRF_FRAME_INFO                  /*func*/)
{
    _host->AddTestTailcallPoint(gsl::narrow<ULONG>(clientData));
}
